...
class ImageGenerator:
    ...
    def _generate_placeholder_avatar(self, identifier: str, size: int) -> Image.Image:
        """生成基于标识符的彩色圆形占位头像，显示首字符"""
        color = self._AVATAR_PALETTE[hash(identifier) % len(self._AVATAR_PALETTE)]
        img = Image.new("RGBA", (size, size), (0, 0, 0, 0))
        draw = ImageDraw.Draw(img)
        draw.ellipse([0, 0, size - 1, size - 1], fill=color)

        char = (identifier[0] if identifier else "?").upper()
    ...
    async def _get_avatar_images(
        self,
        user_ids: List[str],
        nicknames: List[str],
        size: int,
    ) -> List[Image.Image]:
        """获取用户头像列表；QQ 平台优先尝试 QQ 头像接口，失败则生成占位头像"""

        async def fetch_one(session, user_id: str) -> Optional[bytes]:
        ...
        for uid, nickname, raw in zip(user_ids, nicknames, raw_list):
            avatar = None
            if raw:
                avatar = self.process_avatar_data(raw, size)
            if avatar is None:
                identifier = nickname if nickname else uid
                avatar = self._generate_placeholder_avatar(identifier, size)
    ...
    async def generate_schedule_image(self, courses: List[Dict], date_type: str = "today") -> str:
        ...
        nicknames = [course.get("nickname", uid) for uid, course in zip(user_ids, courses)]
        avatars = await self._get_avatar_images(user_ids, nicknames, c.GS_AVATAR_SIZE)
    ...
    async def generate_ranking_image(
        ...
        nicknames = [data.get("nickname", uid) for uid, data in zip(user_ids, ranking_data)]
        avatars = await self._get_avatar_images(user_ids, nicknames, c.RANKING_AVATAR_SIZE)
