...
class ScheduleHelper:
    ...
    async def get_schedule_for_date(self, event, target_date, date_description, session_id: str = None):
        """根据指定日期获取个人课程安排，包含完整的用户验证逻辑"""
        user_id = event.get_sender_id()

        # 向后兼容：若未传入 session_id，则从 event 中推断
        if session_id is None:
            group_id = event.get_group_id()
            session_id = group_id if group_id else f"private_{user_id}"

        if (
            session_id not in self.user_data
            or user_id not in self.user_data[session_id].get("users", {})
        ):
            return None, "你还没有绑定课表哦，请发送 /绑定课表 指令，然后发送 .ics 文件来绑定。"
        ...
        for course in target_courses:
            nickname = (
                self.user_data[session_id]["users"]
                .get(user_id, {})
